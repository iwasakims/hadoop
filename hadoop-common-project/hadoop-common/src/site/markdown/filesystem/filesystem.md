...
Create snapshot with the name automatically generated by system on `path`.
...
Create snapshot with the name specified by `snapshotName` on `path`.
...
Change the name of snapshot on `path` from `snapshotOldName` to `snapshotNewName`.
...
Delete the snapshot named `snapshotName` on `path`.
